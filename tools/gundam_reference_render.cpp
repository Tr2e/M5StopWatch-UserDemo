...
int main(int argc,char** argv){
    ...
    const Shot strikeShots[]={
        ...
        {"eyes-above",-.45f,.30f,2.48f,430,Pose::Display,false,false,0,Part::Head},
        {"eyes-below",-.45f,-.25f,2.48f,430,Pose::Display,false,false,0,Part::Head},
        {"eyes-opposite",.8f,.02f,2.48f,430,Pose::Display,false,false,0,Part::Head},
    ...
    const Shot destinyShots[]={
        {"standing",-.40f,.04f,1.72f,126,Pose::Display,true,false},{"front",0,.02f,1.72f,132,Pose::Display,false,false},
        {"rear",3.14159265f,.02f,1.72f,132,Pose::Display,false,false},{"rear-equipped",3.14159265f,.05f,1.72f,126,Pose::Display,true,false},
        {"side",1.57079633f,.02f,1.72f,132,Pose::Display,false,false},{"other-side",-1.57079633f,.02f,1.72f,132,Pose::Display,false,false},
        {"gray",-.40f,.02f,1.72f,132,Pose::Display,false,true},{"gray-equipped",-.40f,.02f,1.72f,126,Pose::Display,true,true},
        {"face-front",0,0,2.76f,270,Pose::Display,false,false,0,Part::Head},{"face-quarter",-.8f,.02f,2.76f,270,Pose::Display,false,false,0,Part::Head},
        {"eyes-above",-.45f,.30f,2.48f,430,Pose::Display,false,false,0,Part::Head},
        {"eyes-below",-.45f,-.25f,2.48f,430,Pose::Display,false,false,0,Part::Head},
        {"eyes-opposite",.8f,.02f,2.48f,430,Pose::Display,false,false,0,Part::Head},
        {"face-side",-1.57079633f,0,2.76f,270,Pose::Display,false,false,0,Part::Head},{"head-mount",-.7f,.02f,2.14f,390,Pose::Display,false,false,0,Part::Torso,Part::Head},
        {"chest",-.4f,.02f,1.70f,500,Pose::Display,false,false,0,Part::Torso},{"chest-front",0,.02f,1.70f,500,Pose::Display,false,false,0,Part::Torso},
        {"leg",-.4f,.02f,.62f,390,Pose::Display,false,false,.43f,Part::Feet,Part::Thighs,1},{"leg-side",1.57079633f,.02f,.62f,390,Pose::Display,false,false,.43f,Part::Feet,Part::Thighs,1},
        {"arm",-.4f,.02f,1.48f,400,Pose::Display,false,false,-.82f,Part::Shoulders,Part::Hands,-1},
        {"wings-front",0,.18f,1.86f,125,Pose::Display,true,false,0,Part::Aile},{"wings-rear",3.14159265f,.18f,1.86f,125,Pose::Display,true,false,0,Part::Aile},
        {"backpack",2.75f,.20f,1.78f,255,Pose::Display,true,false,0,Part::Backpack,Part::Aile},
        {"rifle",-.70f,.02f,1.10f,285,Pose::Display,true,false,-1.05f,Part::Rifle},{"shield",-.20f,.02f,1.30f,320,Pose::Display,true,false,1.05f,Part::Shield},
        {"sword",2.70f,.02f,1.42f,250,Pose::Display,true,false,-.22f,Part::Sabers},{"cannon",-2.70f,.02f,1.58f,250,Pose::Display,true,false,.22f,Part::Bazooka},
        {"grip-assembly",-.8f,.08f,1.12f,430,Pose::Display,true,false,-.92f,Part::Arms,Part::Rifle,-1},
        {"shield-mount",-2.15f,.10f,1.34f,400,Pose::Display,true,false,1.0f,Part::Shoulders,Part::Shield,1},
        {"weapon-mounts",3.14159265f,.08f,1.66f,300,Pose::Display,true,false,0,Part::Backpack,Part::Bazooka},
        {"top",-.4f,.70f,1.72f,116,Pose::Display,true,false},{"underside",-.4f,-.30f,1.72f,116,Pose::Display,true,false},
    };
    const Shot* shots=destiny?destinyShots:strike?strikeShots:nu?nuShots:zaku?zakuShots:sazabi?sazabiShots:rxShots;
    const size_t shotCount=destiny?sizeof(destinyShots)/sizeof(Shot):strike?sizeof(strikeShots)/sizeof(Shot):nu?sizeof(nuShots)/sizeof(Shot):zaku?sizeof(zakuShots)/sizeof(Shot):sazabi?sizeof(sazabiShots)/sizeof(Shot):sizeof(rxShots)/sizeof(Shot);
    for(size_t shot=0;shot<shotCount;++shot){const auto& s=shots[shot];
        if(destiny)buildDestinyGundam(*mesh,{s.equipment,false,s.gray,s.pose},static_cast<DestinyStage>(stage));
        else if(strike)buildStrikeGundam(*mesh,{s.equipment,false,s.gray,s.pose},static_cast<StrikeStage>(stage));

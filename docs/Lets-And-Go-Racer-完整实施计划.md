...
> 当前优先级：R26 代码基线固定；下一阶段将交互与 UI 作为最高优先级，按开赛流程、按钮/触屏语义、比赛 HUD、暂停与结算逐步优化。见 [R27 交互与 UI 优化计划](Lets-And-Go-Racer-R27-交互与UI优化计划.md)。

> R26：新增机身按键/触屏操作，修复 RGB 输入阻塞、菜单短拨和恢复门禁，并开始实测驱动的渲染优化。当前第十五版恢复实心跑道，最多三辆车（两名对手），替代下文历史四车方案；赛车画质保持现状。见 [R26 机身操作与性能优化](Lets-And-Go-Racer-R26-机身操作与性能优化.md)。

> 外设输入维护入口：[外设输入接入与故障排查规范](外设输入接入与故障排查规范.md)。记录 `4d2d477` 的供电、时钟、慢帧按键修复，以及 RGB 总线阻塞、菜单短拨和恢复门禁的待整改项；历史阶段“通过”不替代当前故障注入与真机验收。

> 新增／优化车型的固定入口：[车型建模与优化规范](Lets-And-Go-Racer-车型建模与优化规范.md)。按参考版本、独立结构、逐车两轮自审、生产渲染与资源门禁执行。

> R25 已将赛车建模／贴图／优化全过程提取为 [模型制作资产库](模型制作资产库.md)：可调用技能、结构与材质卡、生产共用部件、八车证据清单及像素回归工具。现有造型不变，详见 [R25 提取与两轮审查](Lets-And-Go-Racer-R25-模型制作资产提取.md)。

> R24 按 R17／R18 方法逐台复核新增四车：Cobra 腰线与双进气口、Spider 硬边窗与侧导轮、Stinger 刀脊与四喇叭口、Diospada 侧进气道与实开槽拱翼。同期修正展示页主机测试的车型设置盲区。见 [R24 逐台结构优化](Lets-And-Go-Racer-R24-新增四车逐台结构优化.md)。

> R23 新增 Spin Cobra、Beak Spider、Ray Stinger、Diospada，当前共 8 台可选赛车；车库四视角与比赛均使用各自独立实体模型，对手仍最多 3 名。参考、两轮自审与验证见 [R23 新增四台赛车](Lets-And-Go-Racer-R23-新增四台赛车.md)。

> R22 新增 TRI CROSS 02 三交叉立交赛道，现有 2 条可选赛道；左右选道，仍跑三圈。同步接入比赛、小地图、独立圈速记录和旧存档迁移，原 SKY LOOP 01 保留。实现与两轮自审见 [R22 第二条立交赛道](Lets-And-Go-Racer-R22-第二条立交赛道.md)。

> R21 增加选车页四视角与 350 ms 平滑切换；左右换车、上下换视角，只有到达侧视图后车轮才转动。斜前视保留轻微自动摆动，比赛轮动不变。见 [R21 交互视角与侧视轮动](Lets-And-Go-Racer-R21-交互视角与侧视轮动.md)。此前 [R20 8bit 音乐与音效](Lets-And-Go-Racer-R20-8bit音乐与音效.md) 保留。

> R19 对照田宫 JCJC 实拍重做红蓝白模块、实体侧墙、接缝与支撑，选道／比赛改深色远景，小地图改为 96 段实体色带。两轮自审修复近景墙面遮挡、小地图条纹与安全边距、目标平台类型差异；玩法和赛车不变。当前赛道与资源预算见 [R19 田宫赛道与深色远景](Lets-And-Go-Racer-R19-田宫赛道与深色远景.md)。
...
2. 从八辆官方车型中选择玩家车辆。
3. 以四个预设视角和平滑切换预览实体 3D 车辆，侧视图展示轮毂旋转；比赛沿用同源模型。
4. 从剩余车辆中选择 0～3 名对手。
5. 在赛道页选择两条无中间隔板立交赛道之一。
...
| Spin Cobra | `spin_cobra` | 蓝色膨胀轮罩、黄红火焰、三辐金色轮毂 | 转向/响应 |
| Beak Spider | `beak_spider` | 黑色蛛网车壳、金色座舱、三层尾翼、红色盘式轮毂 | 直线 |
| Ray Stinger | `ray_stinger` | 银色尖鼻、成对金色进气口、单片中央尾鳍 | 极速/加速 |
| Diospada | `diospada` | 红色跑车轮罩、开放侧进气道、拱形尾翼 | 稳定/转向 |
...
| Joystick Y | 选车页上下循环斜前／侧／后／俯视；其余页不映射 | 不映射，避免误操作 |
| Blue 短按 | 确认/添加对手 | 无 |
| Blue 按住 | 无 | Boost |
| Red 短按 | 取消/移除对手 | 刹车 |
| Red 长按 | 返回上一页 | 暂停/继续 |

...
> R20 加入原创 8-bit BGM、最后一圈提速、结算变奏与 14 类音效；既有音频任务内分块混音，遵守系统音量／SFX 开关。两轮自审修复暂停噪声状态、提示音抢占、退出回调及振动参数顺序。见 [R20 8bit 音乐与音效](Lets-And-Go-Racer-R20-8bit音乐与音效.md)，含同源试听。当前 30 套回归，设备音质与欠载待验收。

...
> R18 按 R17 同样流程完成剩余三车，分别修正 Sonic 连接翼与阶梯尾部、Neo 三叉鼻/分窗座舱/后掠翼/轮毂与侧导轮、Brocken 三段主体/管架/六辐轮毂；每车两轮自检和阶段提交，加入附件穿胎和比赛包围盒回归。当前画面与预算见 [R18 其余三车结构复核](Lets-And-Go-Racer-R18-其余三车结构复核.md)。

> R17 单独重建第一辆 Cyclone Magnum 的宽肩/窄腰、开放侧通道、硬边座舱和独立轮罩，修复轮罩下沿穿胎与比赛涂装镜像；新增三档结构约束和多视角检查。当前第一辆车见 [R17 Magnum 腰线与结构复核](Lets-And-Go-Racer-R17-Magnum腰线与结构复核.md)。

> R16 根据用户允许放弃 wireframe 的新要求，车库与比赛均改用同源实体曲面、UV 材质贴花和逐像素深度遮挡，重新校对四车轮廓；以下车辆渲染约束已更新，R0–R15 阶段记录保留为历史。实际画面、两轮自审和资源预算见 [R16 实体赛车与比赛细化](Lets-And-Go-Racer-R16-实体赛车与比赛细化.md)。真机帧率仍待验收。

> R15 增强无隔板跑道配色、路肩、桥体厚度与遮挡、选道视角和 HUD 对比；不改赛道物理。最新赛道画面及两轮自审见 [R15 跑道视觉优化](Lets-And-Go-Racer-R15-跑道视觉优化.md)。

> R14 已按田宫实物照片重建四车部件与配色，纠正 Sonic 尾翼和 Brocken 主色的早期误读。当前车辆视觉、两轮自审和预算见 [R14 实车模型重建](Lets-And-Go-Racer-R14-实车模型重建.md)；早期阶段文档保留为历史。
...
在 StopWatch 上实现一个完整的手绘色彩迷你四驱车比赛闭环，采用实体赛车与矢量赛道：

1. 进入 App 后检测并校准 Joystick2 + Dual Button。
2. 从四辆官方车型中选择玩家车辆。
3. 以可连续旋转的实体 3D 曲面预览车辆，展示轮毂旋转与逐风效果；比赛沿用同源模型。
4. 从剩余车辆中选择 0～3 名对手。
...
- 车库使用 High 实体网格（1171～1361 面），比赛默认使用 Medium（955～1139 面），压力时降到 Low（739～917 面），均保留实体车壳与材质，不回退线框。R17–R18 四车均采用独立截面与部件连接，管件和轮胎按档位减少圆周采样。
- 车辆使用独立颜色/深度分块缓存，透视正确 UV 与近裁剪；桥面仍参与逐像素遮挡。缓存进入游戏分配、退出释放，详见 R16 预算。
- 只旋转轮毂叶片，保持轮胎包络稳定；不使用视频帧或角度序列图。

...
public:
    ...
    void setSharedUvReciprocal(bool enabled) { _sharedUvReciprocal=enabled; }
    ...
    void triangle(CarScreenVertex a,CarScreenVertex b,CarScreenVertex c,
        ...
        for(int y=y0;y<=y1;++y) {
            ...
            if(paint!=CarPaint::Solid) {
                const float uNumerator=a.uDepth+s*(b.uDepth-a.uDepth)+t*(c.uDepth-a.uDepth);
                const float vNumerator=a.vDepth+s*(b.vDepth-a.vDepth)+t*(c.vDepth-a.vDepth);
                float u,v;
                if(_sharedUvReciprocal) {
                    const float reciprocal=1.f/depth;
                    u=uNumerator*reciprocal;v=vNumerator*reciprocal;
                } else { u=uNumerator/depth;v=vNumerator/depth; }
...
private:
    bool _sharedUvReciprocal=false;

...
int main(int argc, char** argv)
{
    ...
    for(unsigned car=0;car<kCarCount;++car) {
        ...
        for(int sample=0;sample<4;++sample) {
            ...
            hero.setSharedUvReciprocal(false);
            ...
            hero.setSharedUvReciprocal(true);
            hero.render(drive,run,results,0,false,detail,true);
            unsigned uvChanges=0;
            for(std::size_t pixel=0;pixel<solidFrame.size();++pixel)uvChanges+=solidFrame[pixel]!=canvas.frame()[pixel];
            std::cout<<"UV reciprocal car="<<car<<" sample="<<sample<<" changed_pixels="<<uvChanges<<'\n';
            if(sample==0)save("uv-reciprocal-"+std::to_string(car));
            hero.setSharedUvReciprocal(false);

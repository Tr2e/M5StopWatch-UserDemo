...
void drawRaceCar(LGFX_Sprite& canvas,const TrackCamera& nativeCamera,
                 ...
                 float occlusionScale=1.f,int rasterPercent=100)
{
    TrackCamera camera=nativeCamera;
#ifdef ESP_PLATFORM
    ...
    const int tileSide=(112*rasterPercent+50)/100;
    if(rasterPercent!=100) {
        const float ratio=float(tileSide)/112;
        camera.principalX=(camera.principalX-x0)*ratio;
        camera.principalY=(camera.principalY-y0)*ratio;camera.focalLength*=ratio;
        for(std::size_t i=0;i<count;++i) {
            auto& p=prepared[i];p.left=(p.left-x0)*ratio;p.right=(p.right-x0)*ratio;
            p.top=(p.top-y0)*ratio;p.bottom=(p.bottom-y0)*ratio;
            if(p.visibility==1)for(auto& vertex:p.screen) {vertex.x=(vertex.x-x0)*ratio;vertex.y=(vertex.y-y0)*ratio;}
        }
    }
#ifdef ESP_PLATFORM
    ...
    for(int y=y0;y<=y1;y+=112) for(int x=x0;x<=x1;x+=112) {
...
#endif
        if(rasterPercent==100)raster.begin(x,y);
        else raster.begin((x-x0)/112*tileSide,(y-y0)/112*tileSide,tileSide,tileSide);
        ...
        ++tiles;
#endif
        if(rasterPercent==100)raster.blit(canvas,&occlusion,occlusionScale);
        else raster.blitScaled(canvas,x,y,112,112,&occlusion,occlusionScale);
#ifdef ESP_PLATFORM
...
{
    _stages={};
    _playerPaintAtlas.reset();_playerPaintCar=CarId::Count;
    _playerRasterPercent=100;_usePlayerPaint=false;_playerPaintMisses=0;_playerPaintBuildUs=0;
    _bulkSceneCopy=true;
...
void RaceRenderer::close()
{
    ...
    _surface.reset();
    _scene.reset();
    _paintAtlas.reset();_miniMapImage.reset();_playerPaintAtlas.reset();_playerPaintCar=CarId::Count;
}
                          ...
                          PencilDetail detail, bool deviceControls)
{
    ...
    if(_playerQuality && _usePlayerPaint && _playerPaintCar!=race.snapshot().player().car) {
#ifdef ESP_PLATFORM
        const auto start=esp_timer_get_time();
#endif
        if(!_playerPaintAtlas)_playerPaintAtlas.reset(new(std::nothrow) PlayerPaintAtlas);
        if(_playerPaintAtlas) {
            _playerPaintAtlas->clear();_playerPaintMisses=0;
            const auto& mesh=_surface->meshes[race.snapshot().playerIndex];
            for(std::size_t i=0;i<mesh.count;++i) {
                const auto& face=mesh.panels[i];
                if(!_playerPaintAtlas->add(face.paint,face.color,face.light))++_playerPaintMisses;
            }
            _playerPaintCar=race.snapshot().player().car;
        }
#ifdef ESP_PLATFORM
        _playerPaintBuildUs=esp_timer_get_time()-start;
#endif
    }
    ...
    const auto submit=[&](std::size_t index,LGFX_Sprite& target,const TrackCamera& view,float scale) {
        ...
        if(_playerQuality && car.player && _usePlayerPaint)_surface->raster.setPlayerPaintAtlas(_playerPaintAtlas.get());
        else _surface->raster.setPaintAtlas(_playerQuality && car.player ? nullptr : _paintAtlas.get());
#ifdef ESP_PLATFORM
        ...
        drawRaceCar(target,view,race.track(),car,_surface->meshes[index],_surface->raster,
            ...
            _playerQuality && car.player ? _playerRasterPercent : 100);
        ++submitted;
#ifdef ESP_PLATFORM
    ...
    if (_scene) {
        ...
        direct=direct && canvas.getColorDepth()==16 && canvas.getRotation()==0 &&
            scene.getColorDepth()==16 && scene.getRotation()==0 &&
            clipX==0 && clipY==0 && clipW==_width && clipH==_height;
#endif
        ...
        for (int y = 0; y < scene.height(); ++y) {
            ...
            if(direct) {
                std::memcpy(destination+(y*2)*_width,row.data(),_width*sizeof(ScenePixel));
                std::memcpy(destination+(y*2+1)*_width,row.data(),_width*sizeof(ScenePixel));
            } else {
    ...
    _stages={uint32_t(trackFinishedUs-trackStartedUs),playerUs,opponentsUs,
        uint32_t(upscaleFinishedUs-carsFinishedUs),uint32_t(esp_timer_get_time()-hudStartedUs)};
#endif

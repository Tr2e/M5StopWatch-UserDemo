...
bool validateCarRaster()
{
    ...
    canvas.fillScreen(0xef3a);raster.blitScaled(canvas,180,180,32,32,&occlusion);
    if(nativeOcclusion!=canvas.frame()) {std::cerr<<"Scaled composite lost native occlusion\n";valid=false;}
    ...
    canvas.fillScreen(0xef3a);raster.blitScaled(canvas,180,180,32,32,smallOcclusion.get(),.5f);
    if(nativeOcclusion!=canvas.frame()) {std::cerr<<"Scaled composite lost half-scene occlusion\n";valid=false;}
    occlusion.count=0;
...
int main(int argc, char** argv)
{
    ...
    for(unsigned car=0;car<kCarCount;++car) {
        ...
        for(int sample=0;sample<4;++sample) {
            ...
            for(const auto mode:{std::pair<int,bool>{90,false},{85,false},{100,true},{90,true},{85,true}}) {
                hero.setPlayerRenderMode(mode.first,mode.second);
                hero.render(drive,run,results,0,false,detail,true);
                if(sample==0)save("quality-"+std::to_string(car)+"-"+std::to_string(mode.first)+(mode.second ? "-atlas" : "-paint"));
                for(int y=36;y<73;++y)for(int x=119;x<346;++x)
                    if(solidFrame[y*468+x]!=canvas.frame()[y*468+x]) {std::cerr<<"Player quality changed HUD\n";valid=false;}
            }
            std::cout<<"Player atlas car="<<car<<" count="<<hero.playerPaintCount()<<" fallback_panels="<<hero.playerPaintMisses()<<'\n';
            hero.setPlayerRenderMode(100,false);hero.render(drive,run,results,0,false,detail,true);
            if(solidFrame!=canvas.frame()) {std::cerr<<"Player quality failed native restoration\n";valid=false;}

...
public:
    ...
    void setPaintAtlas(const RacePaintAtlas* atlas) { _paintAtlas=atlas;_playerPaintAtlas=nullptr; }
    void setPlayerPaintAtlas(const PlayerPaintAtlas* atlas) { _playerPaintAtlas=atlas;_paintAtlas=nullptr; }
    ...
    void triangle(CarScreenVertex a,CarScreenVertex b,CarScreenVertex c,
        ...
        const auto* texture=_playerPaintAtlas ? _playerPaintAtlas->find(paint,color,light) :
            _paintAtlas ? _paintAtlas->find(paint,color,light) : nullptr;
        ...
        for(int y=y0;y<=y1;++y) {
            ...
            if(paint!=CarPaint::Solid) {
                ...
                pigment=texture ? (_playerPaintAtlas ? PlayerPaintAtlas::sample(texture,u,v) : RacePaintAtlas::sample(texture,u,v)) :
    ...
    void blitScaled(LGFX_Sprite& canvas,int x,int y,int width,int height,
                    const PencilOcclusion* occlusion=nullptr,float occlusionScale=1.f) const {
        ...
        std::array<uint16_t,PencilOcclusion::kCapacity> candidates{};std::size_t count=0;
        if(occlusion)for(std::size_t i=0;i<occlusion->count;++i) {
            const auto& s=occlusion->surfaces[i];
            if(s.maxX>=x*occlusionScale && s.minX<(x+width)*occlusionScale &&
               s.maxY>=y*occlusionScale && s.minY<(y+height)*occlusionScale) {
                float area=0;
                for(std::size_t edge=0;edge<s.count;++edge) {
                    const auto a=s.points[edge],b=s.points[(edge+1)%s.count];area+=a.x*b.y-a.y*b.x;
                }
                candidates[count++]=uint16_t(i)|(area<0 ? 0x8000u : 0u);
            }
        }
        for(int py=0;py<height;++py) {
            ...
            for(int px=0;px<=width;++px) {
                bool visible=px<width && depth[offset+sourceX[px]]!=0;
                if(visible && occlusion) {
                    const float inverse=depth[offset+sourceX[px]]/8192.f;
                    const TrackScreenPoint p{(x+px+.5f)*occlusionScale,(y+py+.5f)*occlusionScale};
                    for(std::size_t i=0;i<count;++i) {
                        const auto packed=candidates[i];const auto& s=occlusion->surfaces[packed&0x7fffu];
                        if(s.count<3 || p.x<s.minX || p.x>s.maxX || p.y<s.minY || p.y>s.maxY)continue;
                        const auto& d=s.inverseDepth;
                        if(d.x*p.x+d.y*p.y+d.z-inverse-.0001f<0)continue;
                        const float orientation=(packed&0x8000u) ? -1.f : 1.f;
                        bool inside=true;
                        for(std::size_t edge=0;edge<s.count;++edge) {
                            const auto a=s.points[edge],b=s.points[(edge+1)%s.count];
                            if(orientation*((b.x-a.x)*(p.y-a.y)-(b.y-a.y)*(p.x-a.x))<0) {inside=false;break;}
                        }
                        if(inside) {visible=false;break;}
                    }
                }
...
private:
    ...
    const PlayerPaintAtlas* _playerPaintAtlas=nullptr;

...
public:
    ...
    void close();
    void setPlayerRenderMode(int percent,bool cachedPaint) {
        _playerRasterPercent=percent==85 ? 85 : percent==90 ? 90 : 100;_usePlayerPaint=cachedPaint;
    }
    unsigned playerPaintCount() const { return _playerPaintAtlas ? _playerPaintAtlas->count() : 0; }
    unsigned playerPaintMisses() const { return _playerPaintMisses; }
    uint32_t playerPaintBuildUs() const { return _playerPaintBuildUs; }
...
private:
    ...
    std::unique_ptr<PlayerPaintAtlas> _playerPaintAtlas;
    CarId _playerPaintCar=CarId::Count;
    int _playerRasterPercent=100;
    bool _usePlayerPaint=false;
    unsigned _playerPaintMisses=0;
    uint32_t _playerPaintBuildUs=0;

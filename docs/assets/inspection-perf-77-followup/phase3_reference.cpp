...
void drawCar(LGFX_Sprite& canvas,const CarSpec& spec,const CarDisplayMesh& mesh,
             Phase3ReferenceSurfaceCache& surface,int centerX,int centerY,float scale,float yaw,
             float wheelPhase,PencilDetail,float pitch=.5713375f,bool pitTheme=false,
             int percent=100,bool inspectionPrepared=false) {
#ifdef ESP_PLATFORM
    ...
    const int originX=centerX-176,originY=centerY-162;
    ...
    else raster.blitScaled(canvas,originX,originY,352,288);
#ifdef ESP_PLATFORM
    const uint64_t endUs=esp_timer_get_time();
    static uint64_t lastLogUs=0;
    if(endUs-lastLogUs>=2000000u) {
        lastLogUs=endUs;
        mclog::tagInfo("ReferenceGarageStage","panels={} vertices={} scale_pct={} clear_us={} shadow_us={} project_us={} raster_us={} blit_us={}",
...
void Phase3ReferenceRenderer::open(int width, int height)
{
    _width = width;
    _height = height;
    _meshCached = false;
    _inspectionPercent = 100;
    _surface.reset(new(std::nothrow) Phase3ReferenceSurfaceCache);
    if(_surface)_surface->trackSurfaces.preferInternalMemory();
    _trackPreview.open(_track);
}

void Phase3ReferenceRenderer::close()
{
    _width = 0;
    _height = 0;
    _meshCached = false;
...
const CarDisplayMesh& Phase3ReferenceRenderer::showcaseMesh(CarId car, PencilDetail detail)
{
...
void Phase3ReferenceRenderer::render(const GameFlow& flow, const GarageSelection& selection,
                            ...
                            int inspectionPercent)
{
    ...
    if (screen == GameScreen::CarInspect) {
        ...
        _inspectionPercent=inspectionPercent==77 ? 77 : inspectionPercent==80 ? 80 : inspectionPercent==85 ? 85 :
        ...
        if(modelFinishedUs-lastInspectionLogUs>=2000000u) {
            ...
            mclog::tagInfo("ReferenceInspectionStage","car={} scale_pct={} fit_us={} model_us={}",
    ...
    if (screen == GameScreen::CarSelect) {
        ...
        drawCar(canvas, spec, mesh(), *_surface, _width / 2+std::lround(view.carSlide),

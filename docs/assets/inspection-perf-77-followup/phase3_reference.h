...
#include "phase3_reference_raster.h"
...
struct Phase3ReferenceSurfaceCache {
    CarDisplayMesh mesh{};
    Phase3ReferenceRaster<352,288> raster{};
    PencilOcclusion trackSurfaces{};
    GarageInspectionCache inspection{};
};
static_assert(sizeof(Phase3ReferenceSurfaceCache)<=724000u,"garage surface working-set budget");

class Phase3ReferenceRenderer {
public:
...
private:
    ...
    std::unique_ptr<Phase3ReferenceSurfaceCache> _surface;
...
static_assert(sizeof(Phase3ReferenceRenderer) <= 3500u,

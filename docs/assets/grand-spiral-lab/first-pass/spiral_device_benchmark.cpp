...
void spiralDeviceBenchmark() {
    ...
    for(unsigned car=0;car<kCarCount;++car)for(auto track:{TrackId::SkyLoop,TrackId::TriCross,TrackId::GrandSpiral}) {
        ...
        if(car==0)for(unsigned frame=0;frame<13;++frame) {
            ...
            if(frame)mclog::tagInfo("SpiralPreview","track={} frame={} draw={} present={}",int(track),frame,uint32_t(drawn-start),uint32_t(end-drawn));

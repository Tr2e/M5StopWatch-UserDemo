...
void occlusionDeviceBenchmark() {
    ...
    mclog::tagInfo("OcclusionAB","BEGIN internal={} psram={}",heap_caps_get_free_size(MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT),heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    for(unsigned car=0;car<kCarCount;++car)for(auto track:{TrackId::SkyLoop,TrackId::TriCross}) {
        ...
        for(int frame=0;frame<25;++frame) {
            ...
            for(unsigned i=0;i<state.carCount;++i) {
                auto& c=state.cars[i];c.motion.distance=frame*race->track().length()/24;c.motion.speed=12.f;
            ...
            for(int pass=0;pass<2;++pass) {
                const int mode=(frame+pass)%2;renderer->setRowOcclusionFilter(mode==1);
            ...
            if(frame) for(int mode=0;mode<2;++mode) {
                const auto& s=stages[mode];const auto& w=s.cars.work;
                mclog::tagInfo("OcclusionAB","car={} track={} frame={} mode={} draw={} present={} prepare={} raster={} blit={} player={} opponents={} candidates={} before={} after={} rows={}",
                    car,int(track),frame,mode,draw[mode],present[mode],s.cars.prepareUs,s.cars.rasterUs,s.cars.blitUs,s.playerUs,s.opponentsUs,w.tileCandidates,w.unfilteredRowCandidates,w.rowCandidates,w.rows);
            }
        }
        mclog::tagInfo("OcclusionAB","GROUP car={} track={} mismatches={} stack_free={}",car,int(track),mismatches,uxTaskGetStackHighWaterMark(nullptr));
    }
    mclog::tagInfo("OcclusionAB","END mismatches={} internal={} psram={} stack_free={}",mismatches,heap_caps_get_free_size(MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT),heap_caps_get_free_size(MALLOC_CAP_SPIRAM),uxTaskGetStackHighWaterMark(nullptr));

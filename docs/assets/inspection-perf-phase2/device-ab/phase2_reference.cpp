...
void drawCar(LGFX_Sprite& canvas,const CarSpec& spec,const CarDisplayMesh& mesh,
             ReferenceGarageSurfaceCache& surface,int centerX,int centerY,float scale,float yaw,
             float wheelPhase,PencilDetail,float pitch=.5713375f,bool pitTheme=false,
             int percent=100,bool=false) {
    ...
    for(std::size_t i=0;i<mesh.count;++i)raster.panel(camera,mesh.panels[i],rasterTransform);
...
void ReferenceGarageRenderer::open(int width, int height)
{
    _width = width;
    _height = height;
    _meshCached = false;
    _inspectionPercent = 100;
    _surface.reset(new(std::nothrow) ReferenceGarageSurfaceCache);
    if(_surface)_surface->trackSurfaces.preferInternalMemory();
    _trackPreview.open(_track);
}

void ReferenceGarageRenderer::close()
{
    _width = 0;
    _height = 0;
    _meshCached = false;
...
const CarDisplayMesh& ReferenceGarageRenderer::showcaseMesh(CarId car, PencilDetail detail)
{
...
void ReferenceGarageRenderer::render(const GameFlow& flow, const GarageSelection& selection,
                            ...
                            int inspectionPercent)
{
    ...
    if (screen == GameScreen::CarInspect) {
        ...
        _inspectionPercent=inspectionPercent==80 ? 80 : inspectionPercent==85 ? 85 :

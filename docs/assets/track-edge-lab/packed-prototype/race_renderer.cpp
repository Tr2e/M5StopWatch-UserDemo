                          ...
                          PencilDetail detail, bool deviceControls)
{
    ...
    if (_scene) {
        ...
        for (int y = 0; y < scene.height(); ++y) {
            auto* second=row.data();
            if(edgeUpscaleActive()) {
                second=_edgeRow.get()->data();
                upscaleEdgeRow(source+std::max(0,y-1)*scene.width(),source+y*scene.width(),
                    source+std::min<int>(scene.height()-1,y+1)*scene.width(),scene.width(),row.data(),second);

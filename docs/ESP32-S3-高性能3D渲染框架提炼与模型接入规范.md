...
> 项目名：**ThunderRaster（雷栅）**。这是对外的框架名称；现有 `soft3d` C++ namespace 和已签署 ABI 暂不做纯命名重构，避免无性能收益的代码扰动。
...
- depth/color 基址、行偏移和分带 offset 等不随 x 变化的寻址应提升到扫描行边界；仅当存储所有权能证明平面不重叠时才可使用编译器不别名约束。该优化不得重排重心或深度浮点求值，并须通过颜色、量化 depth 与 occupancy 的逐点门禁。
...
| 扫描边 setup | 高覆盖 quad 可在图元 setup 缓存三条边的纵向范围和水平边分类，避免每扫描行重复 `min/max/abs`；保持重心/depth 表达式原样，并以目标机端到端收益和代码/堆余量签署 | 条件必做 |
...
11. 循环不变量不仅包括材质和策略，也包括目标寻址。RX-78 E8 把 depth/color 行指针与 band offset 提升到扫描行入口，在语义不变和严格 depth 门禁通过的前提下，使 60% 帧时再降低约 `0.27%`、100% 降低约 `0.42%`。不别名声明必须来自真实存储所有权，不能作为猜测性的编译提示。
12. 控制流不变量也应按生命周期提升。RX-78 E9 把 quad 三条边的 `minY/maxY` 与水平分类从每条扫描行提升到图元 setup，60% 帧时降低约 `0.87%`、100% 降低约 `1.20%`；这类缓存应紧凑，只保存能消除重复分支/比较的元数据。
13. 不要把已验证的提升机械复制到较短路径。把行指针准备复制到普通三角后，新增逐行固定成本高于省下的索引计算，60% 从 E8 的 `50.705 ms` 退化到 `50.925 ms`。是否复用必须按图元数量、平均 span 和像素覆盖真机 A/B，不按函数外观判断。
14. 主机微基准不能签署 Xtensa 热循环。把六个坐标/深度差加入 quad setup 后，主机约快 7–9%，但真机因寄存器 spill/IRAM 克隆布局变化退化到 60% `55.425 ms`、100% `93.282 ms`。热 setup 增加字段或 live range 时必须同时审计目标机代码布局、internal free 和端到端三轮；主机结果只用于筛选。逐像素递增变量即使颜色相同，也必须通过 Q13 depth 门禁。

...
跳跃仍是程序姿势，不用 mocap（Bandai 集 1 没有 jump）。按 B 后先在地面下蹲蓄力 `kJumpCrouch=0.20s`（约 0.12s 蹲到位再稍停），再离地。空中随弹道蹬直 → 顶点微收 → 下落伸腿；手臂先向后攒力再前上摆。落地 `kJumpLand=0.20s` 屈膝缓冲并平滑回到 Idle。蓄力期间钉住位移。

...
2026-09-17 跳跃专项踩过两次：先把 CMU 关节角 1:1 写进 SD，蓄力几乎没蹲、整机却弹到约 0.6 个身长；再按映射角写臂和空中膝，摆臂只比走路大一点。后续任何走、跳、踢、挥、落地都按这条。
...
核对口诀：蹲完头/髋有没有明显下降；跳起来是不是大约四分之一个机体；摆臂是否明显大于走路；空中收腿看得出但没缩成一团。这四项有一项不对，就是尺度又错了。

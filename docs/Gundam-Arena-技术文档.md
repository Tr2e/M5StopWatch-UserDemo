...
`DNC L` / `DNC S` 不进 A/B 圈。源片整段按 SD 身份钥匙烘焙进独立 `arena_dance_clips.h`（int16 / 10000，4751 / 1901 帧，约 523 KB + 209 KB）。**按帧**用手相对头写举起（留 0.88 余量，不顶死 RAISE），上臂 yaw 用约 1.2s 窗的 WAVE 局部放大（幅 0.42）；双脚着地才下沉（tanh，上限 0.18），单脚离地迈步，双脚离地负 dip 抬 Root。离地 hop 重映射到约 0.31–0.40（接近跳跃 0.44），不按人体脚离地高度 1:1。数据仍是整段；播放从第一次 hop 前约 0.7s 起到**最后一次 hop 后约 0.7s**循环，跳过片头持姿和片尾死区（不再用大腿行程判终点，避免限位持姿被当成迈步）。当前循环窗 L `1258..4351`（约 41.9s–145s）、S `318..1856`（约 10.6s–61.9s）。舞蹈模式内 A/B 只切这两段。`danceMode` 着地时强制保持 `Action::Dance`。Auton 不请求整段舞。
...
### 2026-09-18 · 舞蹈循环裁掉片尾死区

- **范围：** DNC S/L 播到后段会卡住再突然开跳。循环终点用大腿行程误判，把 outro 持姿圈进来（S ~2s，L ~14s）。
- **代码：** `dance_loop_range` 只按第一次/最后一次 hop ± preroll 开闭；片尾不再看 `step_at`。循环窗 L `1258..4351`、S `318..1856`。测试断言终点早于全片尾、循环末 1s 仍有负 dip。
- **已验证：** `gundam_arena ok`。循环窗 L `1258..4351`、S `318..1856`；末跳后仅留 ~0.7s。写入 `/dev/cu.usbmodem83301` MAC `44:1b:f6:c1:8a:00` 应用分区 `0x4c0070` B（4,980,848），余量 4%。esptool `Hash of data verified`，RTS 重启。BIN SHA-256 `5503aad78fbaea90942c5b48b8a843863b1a97b88e56a5878ddb95200069bb72`。ELF SHA-256 前缀 `c5b5e6090` 与启动日志一致。PSRAM 自检通过，Launcher 已起。版本串 `V0.5-226-g41e5a76-dirty`。
- **未验证：** 真机绕回是否还先卡再跳。

...
打开 Racer 后，在检查/校准页按机身 B 或点击屏幕，明确选择 `USE DEVICE`。该模式使用独立的机身按钮和触屏采样，不模拟 Joystick2 已连接，也不跳过外设模式的真实校准。进入机身模式后关闭外设探测与 Grove 供电；晚上接好外设后重新打开 App，等待真实检测/校准即可使用原外设模式。

| 场景 | 机身按键 | 触屏 |
| --- | --- | --- |
| 选择页 | A：下一项；B：确认/切换选中；长按 A：返回，选车首页返回 Launcher | 左/右侧切换，上方标题区返回，中间确认；选车页标题下方区域切换视角 |
| 对手页 | A 切换到 `RACE READY`，B 继续；允许 0～2 名对手 | 两侧切换，中间确认 |
| 比赛 | 按住 A 刹车，按住 B 增压 | y≥130 的区域按下并横向拖动转向；松手回中；点顶部暂停 |
| 暂停 | B：继续；输入来源必须健康 | 松开触屏后重新点击继续 |
| 结算 | A：切换 RETRY/GARAGE/EXIT；B：执行 | 两侧切换，中间确认 |
| 所有页面 | 长按 A+B 800ms：退出 | — |

屏幕切换后先松开按键/触屏再开始新操作，旧手势不会自动触发下一页。菜单同类方向事件允许合并，退出优先。机身 GPIO 每 10ms 采样；内部触屏 I²C 在另一个任务读取，不持有按钮快照锁。外设断线实测仍留待连接 Joystick2 / Dual Button 后执行。
...
建议依次记录：选车正面动态视角、侧面转轮、后/顶静止视角、两条赛道预览、三车比赛、暂停、结算、退出重入。切页后至少停留 5 秒再取样。主机基准运行 `bash tools/benchmark_lets_and_go.sh /tmp/racer-benchmark`，仅用于比较同机算法开销，不能换算为设备 FPS。

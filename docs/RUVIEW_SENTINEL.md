...
4. Read the large left-aligned value as an experimental RF activity score, not a person count or medical measurement.
...
During an activity event, the paragraph under the score shows the peak and elapsed duration. After the room returns to stillness, it keeps the last event age until recalibration.

The live view is a dark, typographic layout: a short status line, a large score, and a muted description on the left. Eight gray dots along the right inner arc represent the eight CSI frequency bands; brightness and size follow per-band deviation, not physical direction. A thin rim arc tracks calibration progress or the current score. Activity, errors, and watch motion show a small marker beside the number, with a two-pulse haptic cue on activity.

If a high activity score persists for at least 12 seconds and then remains stable at the new RF level for 6 seconds, RuView treats it as a lasting room change rather than endless motion. It closes the activity event, shows **ADAPTING**, and learns the new stable environment for 15 seconds. Brief motion never enters this recovery path.
...
- CSI frame rate depends on access-point traffic and firmware behavior. `NO SIGNAL` means the watch is connected but is not receiving usable CSI frames from the AP.
...
- Confirm picking up the watch shows `MOVING`, not `ACTIVITY`.

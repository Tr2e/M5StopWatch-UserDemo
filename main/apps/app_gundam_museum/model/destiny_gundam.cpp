...
constexpr uint16_t white=0xf7be,ivory=0xdedb,blue=0x225b,deepBlue=0x1932;
constexpr uint16_t red=0xd946,gold=0xfe68,frame=0x4a49,black=0x1083,green=0x07ec;
...
void legs(Builder& b,bool detail){
    for(float s:{-1.f,1.f}){
        ...
        b.cover({{-.13f,-.41f,.22f},{.13f,-.41f,.22f},{.17f,-.62f,.30f},{0,-.73f,.34f},{-.17f,-.62f,.30f}},.045f,blue,.008f);
        if(detail){
            b.cover({{-.055f,-.45f,.242f},{.055f,-.45f,.242f},{.07f,-.57f,.302f},{0,-.64f,.329f},{-.07f,-.57f,.302f}},.014f,ivory,.003f);
            b.cover({{s*.15f,-.45f,-.13f},{s*.27f,-.50f,-.20f},{s*.28f,-.68f,-.22f},{s*.12f,-.62f,-.18f}},.04f,blue,.004f);
        }
...
void body(Builder& b,bool detail,DestinyAssembly* a){
    ...
    for(float s:{-1.f,1.f}){
        ...
        b.shell({{-.25f,.17f,.21f},{-.11f,.19f,.22f},{.03f,.15f,.18f}},blue,10);
    }
    b.at(Part::Waist);b.cover({{-.105f,1.39f,.35f},{.105f,1.39f,.35f},{.12f,1.06f,.45f},{0,.99f,.48f},{-.12f,1.06f,.45f}},.05f,white,.007f);
    b.cover({{-.064f,1.36f,.373f},{.064f,1.36f,.373f},{.055f,1.18f,.439f},{-.055f,1.18f,.439f}},.016f,red,.003f);
    ...
    b.cover({{-.15f,1.92f,.34f},{.15f,1.92f,.34f},{.20f,1.63f,.43f},{.11f,1.55f,.45f},{-.11f,1.55f,.45f},{-.20f,1.63f,.43f}},.07f,deepBlue,.008f);
    for(float s:{-1.f,1.f}){
        b.cover({{s*.18f,1.84f,.397f},{s*.43f,1.88f,.34f},{s*.38f,1.68f,.405f},{s*.20f,1.65f,.433f}},.035f,white,.005f);
        if(detail)b.cover({{s*.22f,1.80f,.421f},{s*.38f,1.83f,.381f},{s*.34f,1.73f,.414f},{s*.22f,1.71f,.434f}},.012f,gold,.003f);
    }
    b.cover({{-.095f,1.64f,.446f},{.095f,1.64f,.446f},{.074f,1.48f,.391f},{-.074f,1.48f,.391f}},.025f,red,.004f);
    ...
    b.box(0,1.83f,-.54f,.51f,.45f,.20f,black);
}
...
void head(Builder& b,bool detail,DestinyAssembly* a){
    ...
    b.shell({{2.06f,.34f,.30f,-.02f},{2.18f,.48f,.43f,-.04f},{2.42f,.55f,.49f,-.05f},
        {2.68f,.53f,.47f,-.07f},{2.86f,.44f,.38f,-.09f},{2.98f,.29f,.25f,-.10f},{3.04f,.10f,.10f,-.11f}},ivory,24);
    for(float s:{-1.f,1.f}){
        ...
        b.face({0,2.36f,.49f},{s*.23f,2.37f,.40f},{s*.20f,2.17f,.38f},{0,2.11f,.46f},white,{0,0,1},true);
        b.cover({{s*.035f,2.51f,.455f},{s*.34f,2.55f,.39f},{s*.29f,2.39f,.415f},{s*.09f,2.37f,.47f}},.016f,black,.003f);
        b.cover({{s*.055f,2.493f,.472f},{s*.315f,2.53f,.418f},{s*.278f,2.415f,.435f},{s*.105f,2.397f,.48f}},.008f,green,.002f);
        if(detail){
    ...
    b.cover({{-.072f,2.17f,.445f},{.072f,2.17f,.445f},{.055f,2.07f,.432f},{0,2.025f,.410f},{-.055f,2.07f,.432f}},.035f,red,.006f);
    b.cover({{-.052f,2.96f,.337f},{.052f,2.96f,.337f},{.040f,3.08f,.25f},{-.040f,3.08f,.25f}},.012f,green,.002f);
    // Destiny's paired swept V-fin: gold inner roots and long white outer blades.
    for(float s:{-1.f,1.f}){
        b.cover({{s*.07f,2.78f,.43f},{s*.19f,2.83f,.38f},{s*.46f,3.33f,.19f},{s*.39f,3.35f,.18f},{s*.14f,2.92f,.38f}},.045f,gold,.005f);
        b.cover({{s*.12f,2.74f,.45f},{s*.27f,2.80f,.37f},{s*1.00f,3.42f,.08f},{s*.93f,3.45f,.07f},{s*.23f,2.93f,.37f}},.055f,white,.006f);
    }
...
Point handAnchor(Builder& b,float s){return armAnchor(b,s,{0,-.91f,.10f});}
void arms(Builder& b,bool detail,DestinyAssembly* a){
    for(float s:{-1.f,1.f}){
        ...
        b.cover({{-.23f,.15f,.28f},{.24f,.15f,.28f},{.28f,-.08f,.30f},{.07f,-.22f,.33f},{-.16f,-.18f,.32f}},.045f,white,.006f);
        if(detail)b.cover({{s*.06f,.09f,.32f},{s*.23f,.10f,.31f},{s*.22f,-.04f,.32f},{s*.07f,-.04f,.34f}},.012f,red,.003f);
        ...
        const auto wrist=armAnchor(b,s,{0,-.71f,.06f}),hand=handAnchor(b,s);
...
void backpackAndWeapons(Builder& b,bool detail,DestinyAssembly* a){
    for(int side=0;side<2;++side){const float s=side?1.f:-1.f;
        ...
        const Point main[]={{s*.36f,2.03f,-.66f},{s*.82f,2.42f,-.76f},{s*2.23f,2.72f,-.85f},{s*2.30f,2.52f,-.91f},{s*.91f,1.73f,-.96f},{s*.42f,1.79f,-.79f}};
        prism(b,main,6,.065f,black);
        const Point redTop[]={{s*.47f,2.04f,-.655f},{s*.82f,2.37f,-.745f},{s*2.12f,2.65f,-.83f},{s*1.93f,2.50f,-.835f},{s*.72f,2.25f,-.73f}};
        prism(b,redTop,5,.028f,red);
        const Point lower[]={{s*.48f,1.86f,-.82f},{s*.82f,1.73f,-.91f},{s*1.58f,1.10f,-.92f},{s*1.64f,.88f,-.94f},{s*1.48f,.91f,-.96f},{s*.55f,1.57f,-.91f}};
        prism(b,lower,6,.045f,deepBlue);
        const Point redFin[]={{s*.60f,1.77f,-.79f},{s*.78f,1.68f,-.83f},{s*1.35f,1.23f,-.84f},{s*1.41f,1.04f,-.85f},{s*1.28f,1.08f,-.87f},{s*.63f,1.57f,-.86f}};
        prism(b,redFin,6,.022f,red);
        if(detail)b.tube({s*.48f,1.94f,-.75f},{s*.74f,2.15f,-.79f},.035f,.025f,gold,false,8);
    ...
    b.tube({-.18f,1.70f,-.61f},{-.22f,1.62f,-.82f},.045f,.052f,frame,false,8);
    if(a)a->swordMount.end=b.m.count;
    b.at(Part::Sabers);if(a)a->sword.begin=b.m.count;
    b.tube({-.22f,.60f,-.82f},{-.22f,2.25f,-.82f},.055f,.055f,deepBlue,false,8);
    b.tube({-.22f,2.18f,-.82f},{-.22f,2.52f,-.82f},.07f,.06f,red,false,8);
    b.cover({{-.31f,.70f,-.77f},{-.13f,.70f,-.77f},{-.15f,.48f,-.77f},{-.22f,.37f,-.77f},{-.29f,.48f,-.77f}},.10f,white,.005f);
    if(a)a->sword.end=b.m.count;
    b.at(Part::Backpack);if(a)a->cannonMount.begin=b.m.count;
    b.tube({.18f,1.70f,-.61f},{.22f,1.65f,-.82f},.045f,.052f,frame,false,8);
    if(a)a->cannonMount.end=b.m.count;
    b.at(Part::Bazooka);if(a)a->cannon.begin=b.m.count;
    b.tube({.22f,.75f,-.82f},{.22f,2.46f,-.82f},.075f,.060f,black,true,10);
    b.box(.22f,1.68f,-.82f,.18f,.37f,.16f,deepBlue);
    b.box(.22f,2.24f,-.82f,.15f,.21f,.15f,red);
...
void handheld(Builder& b,DestinyAssembly* a){
    ...
    b.box(0,.20f,.29f,.14f,.18f,.52f,deepBlue);
    ...
    b.cover({{-.22f,.35f,.04f},{.22f,.35f,.04f},{.30f,.06f,.04f},{.16f,-.34f,.04f},{0,-.46f,.04f},{-.16f,-.34f,.04f},{-.30f,.06f,.04f}},.10f,blue,.008f);
    b.cover({{-.16f,.28f,.074f},{.16f,.28f,.074f},{.21f,.06f,.074f},{.09f,-.27f,.074f},{0,-.35f,.074f},{-.09f,-.27f,.074f},{-.21f,.06f,.074f}},.018f,white,.005f);
    b.cover({{-.04f,.24f,.094f},{.04f,.24f,.094f},{.04f,-.27f,.094f},{-.04f,-.27f,.094f}},.012f,red,.003f);

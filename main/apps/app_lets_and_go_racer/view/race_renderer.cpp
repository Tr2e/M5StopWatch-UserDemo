...
void drawRaceCar(lgfx::LGFXBase& canvas,const TrackCamera& camera,
                 ...
                 float occlusionScale,bool filterRows,RaceCarStages& stages)
{
#ifdef ESP_PLATFORM
    ...
    for(int y=y0;y<=y1;y+=112) for(int x=x0;x<=x1;x+=112) {
        ...
        ++tiles;
#endif
        raster.blit(canvas,&occlusion,occlusionScale,filterRows,&stages.work);
#ifdef ESP_PLATFORM
        blitUs+=esp_timer_get_time()-tileDrawUs;
#endif
    }
#ifdef ESP_PLATFORM
    stages.prepareUs+=uint32_t(prepareUs-startUs);
    stages.rasterUs+=uint32_t(rasterUs);stages.blitUs+=uint32_t(blitUs);
                        ...
                        bool wireframeTrack)
{
    _stages={};
    _bulkSceneCopy=true;
    _edgeUpscale=false;_edgeRow={};
    _rowOcclusionFilter=true;
    ...
    if(_surface) {
        ...
        const bool candidates=_surface->raster.preferInternalOcclusionRows();
        if(_scene)_edgeRow.allocate();
#ifdef ESP_PLATFORM
        mclog::tagInfo("RacerMemory","track_rows_internal={} car_planes_internal={} occlusion_rows_internal={} free_internal={} largest_internal={}",
            rows,planes,candidates,heap_caps_get_free_size(MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT),
            heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL|MALLOC_CAP_8BIT));
#else
        (void)rows;(void)planes;(void)candidates;
#endif
    }
                          ...
                          PencilDetail detail, bool deviceControls)
{
    ...
    if (_scene) {
        ...
        if(edges) {
            std::memcpy(edges->source[0].data(),source,scene.width()*sizeof(uint16_t));
            std::memcpy(edges->source[1].data(),source,scene.width()*sizeof(uint16_t));
        }
        for (int y = 0; y < scene.height(); ++y) {
            const ScenePixel* first=row.data();const ScenePixel* second=row.data();
            if(edges) {
                // Packed swap565_t generates byte loads/stores on Xtensa.
                // Copy its unchanged storage bytes into aligned uint16_t rows;
                // rotate source rows so each source line is fetched just once.
    ...
    if(carsFinishedUs-lastLogUs>=2000000u) {
        ...
        mclog::tagInfo("RaceStage","track_us={} cars_us={} upscale_us={} scene_width={} surfaces={} cars={} track_id={} player_car={} submitted={} player_us={} opponents_us={} player_native={} prepare_us={} raster_us={} blit_us={} candidate_rows_before={} candidate_rows_after={}",
            ...
            _surface->occlusion.count,snapshot.carCount,int(race.track().id()),int(snapshot.player().car),submitted,playerUs,opponentsUs,_playerQuality,carStages.prepareUs,carStages.rasterUs,carStages.blitUs,
            carStages.work.unfilteredRowCandidates,carStages.work.rowCandidates);
    }
#else
    (void)submitted;
#endif

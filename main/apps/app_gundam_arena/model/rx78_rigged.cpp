...
#include "../../app_gundam_museum/model/rx78_assembly.h"
#include "../../app_gundam_museum/model/sd_eye_socket.h"
...
class SdBuilder {
public:
    Mesh& m;gundam_museum::BuildOptions options;Part part=Part::Torso;BoneId bone=BoneId::Chest;
    Point origin{};float roll=0,pitch=0,yaw=0;
    void at(Part p,BoneId b,Point o={},float r=0,float t=0,float a=0){
        part=p;bone=b;origin=o;roll=r;pitch=t;yaw=a;
    }
    ...
    void face(Point a,Point b,Point c,Point d,uint16_t color,Point outward,bool insert=false){
        ...
        const std::size_t begin=m.count;
        m.normals[m.count]=n;m.parts[m.count]=part;
    ...
    void tube(Point a,Point b,float r0,float r1,uint16_t color,bool hollow=false,int count=10,bool buriedStart=false){
        auto axis=subtract(b,a);const float len=std::sqrt(dot(axis,axis));if(len<1e-8f)return;
...
void body(SdBuilder& b){
    for(float s:{-1.f,1.f}){
        const float spread=kSpread;
        const Point hip{s*kHipX,kHipY,0};
        const auto thighB=s<0?BoneId::LThigh:BoneId::RThigh;
        const auto shinB=s<0?BoneId::LShin:BoneId::RShin;
        const auto footB=s<0?BoneId::LFoot:BoneId::RFoot;
        b.at(Part::Feet,footB,{hip.x+s*.79f*std::sin(spread),0,0},0,0,s*kFootYaw);
    ...
    for(float s:{-1.f,1.f}){
        ...
        b.at(Part::Waist,BoneId::Pelvis,{s*.37f,1.31f,0},s*.30f,0,s*.35f);
        b.shell({{-.27f,.115f,.225f},{-.03f,.10f,.21f},{.02f,.07f,.18f}},white);
        b.at(Part::Waist,BoneId::Pelvis,{},0,0,pi);
...
void head(SdBuilder& b){
    ...
    for(float s:{-1.f,1.f}){
        ...
        gundam_museum::buildEyeSocket(b,opening,.072f,black,black,gold,nullptr);
        ...
        b.at(Part::Head,BoneId::Head,{s*.466f,2.31f,.255f},0,0,s*.75f);
    ...
    for(int i=0;i<5;++i){auto a=crest[i],c=crest[i+1];
        if(i==0){
            ...
            b.at(Part::Head,BoneId::Head,{},0,0,pi);
...
void armFrame(SdBuilder& b,Part part,BoneId bone,float side){
    b.at(part,bone,{side*.72f,1.90f,0},side*kArmOut,0,0);
}
Point armAnchor(SdBuilder& b,BoneId bone,float side,Point local){
    armFrame(b,Part::Hands,bone,side);auto p=b.transform(local);p.y+=.14f;return p;
}
void forearmFrame(SdBuilder& b,Part part,BoneId bone,float side){
    const auto elbow=armAnchor(b,bone,side,{0,-kUpperArmLen,0});
    b.at(part,bone,elbow,side*kArmOut,kElbowBend,0);
}
Point forearmAnchor(SdBuilder& b,BoneId bone,float side,Point local){
    forearmFrame(b,Part::Hands,bone,side);auto p=b.transform(local);p.y+=.14f;return p;
}
void handFrame(SdBuilder& b,Part part,BoneId bone,float side){
    auto wrist=forearmAnchor(b,bone,side,{0,-kForearmLen,0});
    b.at(part,bone,wrist,side*kArmOut,kElbowBend,0);
}

void arms(SdBuilder& b){
    for(float s:{-1.f,1.f}){
        const auto sh=s<0?BoneId::LShoulder:BoneId::RShoulder;
        const auto ua=s<0?BoneId::LUpperArm:BoneId::RUpperArm;
        const auto fa=s<0?BoneId::LForearm:BoneId::RForearm;
        const auto hd=s<0?BoneId::LHand:BoneId::RHand;
        armFrame(b,Part::Shoulders,sh,s);
        ...
        armFrame(b,Part::Arms,ua,s);
        b.shell({{-.38f,.115f,.12f},{-.15f,.125f,.13f}},white);
        b.tube({-.125f,-kUpperArmLen,0},{.125f,-kUpperArmLen,0},.09f,.09f,frame);
        forearmFrame(b,Part::Arms,fa,s);
        ...
        handFrame(b,Part::Hands,hd,s);
        b.tube({0,.12f,0},{0,0,-.108f},.06f,.058f,frame,false,12);
...
void buildRx78Rigged(Mesh& mesh,Skeleton& bind){
    ...
    gundam_museum::BuildOptions options{};options.equipment=false;
    SdBuilder b{mesh,options};body(b);head(b);arms(b);
    makeBindSkeleton(bind);
    localizeMesh(mesh,bind);
}
} // namespace gundam_arena

...
extern "C" void app_main(void)
{
    ...
    // HAL init
    GetHAL().init();
    network::WifiService::GetInstance().initialize();
...
#if STOPWATCH_BENCHMARK_AUTORUN
    mclog::tagInfo("3DBenchAudit","boot renderer=ThunderRaster autorun app_id={} rx60_only={} band_probe={} topology_stats={}",
                   benchmarkAppId,STOPWATCH_BENCHMARK_RX60_ONLY,STOPWATCH_RX_BAND_PROBE,
                   STOPWATCH_COLLECT_TOPOLOGY_STATS);
    ...
    while (1) {
        GetHAL().feedTheDog();
        network::WifiService::GetInstance().update();
        GetMooncake().update();
    }
}
